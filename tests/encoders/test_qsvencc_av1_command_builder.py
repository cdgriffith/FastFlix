...
def _make_fastflix(encoder_settings, video_settings=None, stream_extras=None):
    """Create a FastFlix instance with QSVEncC AV1-compatible video stream data."""
...
def test_qsvencc_av1_basic_cqp():
    """Test QSVEncC AV1 build with CQP mode produces all-string command list."""
    fastflix = _make_fastflix(
        encoder_settings=QSVEncCAV1Settings(
            bitrate=None,
            cqp=22,
            preset="best",
        ),
    ...
    with mock.patch("fastflix.encoders.qsvencc_av1.command_builder.rigaya_auto_options", return_value=[]):
...
def test_qsvencc_av1_tune_lossless():
...
def test_qsvencc_av1_tune_none():
...
def test_qsvencc_av1_tune_default_is_none():
    """Test that tune defaults to None when not specified."""
    settings = QSVEncCAV1Settings(bitrate=None, cqp=22)
    assert settings.tune is None


def test_qsvencc_av1_with_bitrate():
    """Test QSVEncC AV1 build with VBR bitrate mode."""
    fastflix = _make_fastflix(
        encoder_settings=QSVEncCAV1Settings(
            bitrate="6000k",
            cqp=None,
...
def test_qsvencc_av1_all_elements_are_strings():
